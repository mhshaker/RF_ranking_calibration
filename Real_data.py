...
for seed in range(runs):
    ...
    if calib:
        ...
        # RF ranking + Sig
        sig_rank = _SigmoidCalibration().fit(x_calib_rank, y_calib)
        irrf_cp_sig_test = sig_rank.predict(x_test_rank)
        ...
        ece_irrf_sig = confidance_ECE(convert_prob_2D(irrf_cp_sig_test), y_test, bins=plot_bins)

        ECE_rf_list.append(ece_rf)
        ECE_sig_list.append(ece_sig)
        ECE_iso_list.append(ece_iso)
        ECE_irrf_list.append(ece_irrf)
        ECE_irrf_sig_list.append(ece_irrf_sig)
...
if calib:
    ...
    print("IRRF sig ece ", np.array(ECE_irrf_sig_list).mean())
