...
def split_train_calib_test(name, X, y, tp, test_size, calib_size, seed=0):
...
def calibration(RF, data, calib_methods, metrics):
    ...
    for metric in metrics:
        ...
        for method in calib_methods:
            _dict[method] = []
        results_dict[data["name"] + "_" + metric] = _dict

    # random forest probs
    rf_p_calib = RF.predict_proba(data["x_calib"], laplace=1)
    rf_p_test = RF.predict_proba(data["x_test"], laplace=1)
    results_dict[data["name"] + "_prob"]["RF"] = rf_p_test
    results_dict[data["name"] + "_decision"]["RF"] = np.argmax(rf_p_test,axis=1)
    ...
    if "Platt" in calib_methods:
        ...
        results_dict[data["name"] + "_prob"]["Platt"] = plat_p_test
        results_dict[data["name"] + "_decision"]["Platt"] = np.argmax(plat_p_test,axis=1)
    ...
    if "ISO" in calib_methods:
        ...
        results_dict[data["name"] + "_prob"]["ISO"] = iso_p_test
        results_dict[data["name"] + "_decision"]["ISO"] = np.argmax(iso_p_test,axis=1)

    # RF ranking + ISO
    if "Rank" in calib_methods:
        x_calib_rank = RF.rank(data["x_calib"], class_to_rank=1, train_rank=True)
        x_test_rank = RF.rank_refrence(x_test, class_to_rank=1)
        ...
        results_dict[data["name"] + "_prob"]["Rank"] = rank_p_test
        results_dict[data["name"] + "_decision"]["Rank"] = np.argmax(rank_p_test,axis=1)
    ...
    if "p_rank" in calib_methods:
        ...
        results_dict[data["name"] + "_prob"]["p_rank"] = rank_p_test
        results_dict[data["name"] + "_decision"]["p_rank"] = np.argmax(rank_p_test,axis=1)
    ...
    if "CRF" in calib_methods:
        ...
        results_dict[data["name"] + "_prob"]["CRF"] = crf_p_test
        results_dict[data["name"] + "_decision"]["CRF"] = np.argmax(crf_p_test,axis=1)

    # Venn calibrator
    if "Venn" in calib_methods:
        ven_calib = Venn_calib().fit(rf_p_calib, data["y_calib"])
        ven_p_test = ven_calib.predict(rf_p_test)
        results_dict[data["name"] + "_prob"]["Venn"] = ven_p_test
        results_dict[data["name"] + "_decision"]["Venn"] = np.argmax(ven_p_test,axis=1)

    # Venn abers
    if "VA" in calib_methods:       
        VA = VA_calib().fit(rf_p_calib[:,1], data["y_calib"])
        va_p_test = VA.predict(rf_p_test[:,1])
        results_dict[data["name"] + "_prob"]["VA"] = va_p_test
        results_dict[data["name"] + "_decision"]["VA"] = np.argmax(va_p_test,axis=1)
    ...
    if "Beta" in calib_methods:
        ...
        results_dict[data["name"] + "_prob"]["Beta"] = beta_p_test
        results_dict[data["name"] + "_decision"]["Beta"] = np.argmax(beta_p_test,axis=1)
    ...
    if "Elkan" in calib_methods:
        ...
        results_dict[data["name"] + "_prob"]["Elkan"] = elkan_p_test
        results_dict[data["name"] + "_decision"]["Elkan"] = np.argmax(elkan_p_test,axis=1)
    ...
    if "tlr" in calib_methods:
        ...
        results_dict[data["name"] + "_prob"]["tlr"] = tlr_p_test
        results_dict[data["name"] + "_decision"]["tlr"] = np.argmax(tlr_p_test,axis=1)


    if "acc" in metrics:
        for method in calib_methods:
            results_dict[data["name"] + "_acc"][method].append(accuracy_score(data["y_test"], results_dict[data["name"] + "_decision"][method]))

    if "auc" in metrics:
        for method in calib_methods:
            fpr, tpr, thresholds = roc_curve(data["y_test"], results_dict[data["name"] + "_prob"][method][:,1])
            results_dict[data["name"] + "_auc"][method].append(auc(fpr, tpr))

    if "ece" in metrics:
        for method in calib_methods:
            results_dict[data["name"] + "_ece"][method].append(confidance_ECE(results_dict[data["name"] + "_prob"][method], data["y_test"], bins=plot_bins))

    if "brier" in metrics:
        for method in calib_methods:
            results_dict[data["name"] + "_brier"][method].append(brier_score_loss(data["y_test"], results_dict[data["name"] + "_prob"][method][:,1]))

    if "logloss" in metrics:
        for method in calib_methods:
            results_dict[data["name"] + "_logloss"][method].append(brier_score_loss(data["y_test"], results_dict[data["name"] + "_prob"][method][:,1]))

    if "tce" in metrics:
        for method in calib_methods:
            results_dict[data["name"] + "_tce"][method].append(mean_squared_error(data["tp_test"], results_dict[data["name"] + "_prob"][method][:,1]))
